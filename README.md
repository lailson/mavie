...
# OpenWakeWord Test Interface

Esta é uma interface para testar a integração do sistema de detecção de wake word com o assistente de voz.

## Arquivos Necessários

- **ma_veee.onnx**: Modelo de detecção da wake word
- **positive.wav**: Som de confirmação que é tocado quando a wake word é detectada
- **wakeword_server.py**: Servidor WebSocket que processa a detecção da wake word
- **websocket_tester_ww.html**: Interface web para testar o sistema

## Instruções

1. Execute o servidor de detecção:
   ```
   python wakeword_server.py
   ```
   ou use os scripts auxiliares:
   - Windows: `start_wakeword.bat`
   - Mac/Linux: `./start_wakeword.sh` (você pode precisar dar permissão de execução com `chmod +x start_wakeword.sh`)

2. Abra a interface web no navegador (websocket_tester_ww.html)

3. Diga a palavra de ativação "ma veee" para iniciar a gravação automaticamente

## Troubleshooting

Se não ouvir o som de confirmação, verifique se o arquivo "positive.wav" está no mesmo diretório da interface web.
